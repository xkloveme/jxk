...
class SM4 {
   ...
   * @memberof SM4
   */
  doBlockCrypt(blockData: ArrayLike<number>, roundKeys: number[]) {
   ...
   * @memberof SM4
   */
  padding(originalBuffer: string | any[] | ArrayLike<number> | null) {
    ...
    paddedBuffer.set(originalBuffer as ArrayLike<number>, 0);
   ...
   * @memberof SM4
   */
  dePadding(paddedBuffer: string | any[] | Uint8Array | null) {
   ...
   * @memberof SM4
   */
  uint8ToUint32Block(uint8Array: number[] | Uint8Array | null, baseIndex = 0) {
  ...
  encrypt(plaintext: any) {
    ...
    let blockTimes = (padded && padded.length) || 0 / UINT8_BLOCK;
    let outArray = new Uint8Array((padded && padded.length) || 0);
    if (this.mode === 'cbc') {
      // CBC mode
  ...
  decrypt(ciphertext: any) {
    ...
    return Crypt.utf8ArrayBufferToString(depaddedPlaintext as Uint8Array);
  }
}
