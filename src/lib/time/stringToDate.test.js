...
import time_stringToDate from './stringToDate';

describe('time_stringToDate', () => {
    it("should return a corresponding Date object for the 'YYYY-MM-DD HH:mm:SS' format"),
        () => {
            const date = time_stringToDate('2024-01-01 00:00:00', 'YYYY-MM-DD HH:mm:SS');
    ...
    it("should return a corresponding Date object for the 'YYYY-MM-DD_HH:mm:SS' format"),
        () => {
            const date = time_stringToDate('2024-01-01_00:00:00', 'YYYY-MM-DD_HH:mm:SS');
            expect(date).toEqual(new Date(Date.UTC(2024, 0, 1, 0, 0, 0)));
        };

    it("should return a corresponding Date object for the 'DD/MM/YYYY HH:mm:SS' format"),
        () => {
            const date = time_stringToDate('01/01/2024 00:00:00', 'DD/MM/YYYY HH:mm:SS');
            expect(date).toEqual(new Date(Date.UTC(2024, 0, 1, 0, 0, 0)));
        };

    it("should return a corresponding Date object for the 'MM-DD-YYYY' format"),
        () => {
            const date = time_stringToDate('01-01-2024', 'MM-DD-YYYY');
            expect(date).toEqual(new Date(Date.UTC(2024, 0, 1, 0, 0, 0)));
        };
    ...
    it('should return null for an invalid format'),
        () => {
            ...
            const result = time_stringToDate('01-01-2024', format);
            expect(result).toBeNull();
        };

    it('should return null for a invalid date string'),
        () => {
            const date = time_stringToDate('invalid', 'YYYY-MM-DD HH:mm:SS');
            expect(date).toBeNull();
        };

    it('should correctly adjust for timezone offset'),
        () => {
            const date = time_stringToDate('2024-01-01 00:00:00', 'YYYY-MM-DD HH:mm:SS', -5); // UTC-5
